...
public sealed class AiChatClient
{
    ...
    [
        ...
        new
        {
            ...
            description = "List, read, inspect tree structure, or write the user's DjeLab S3-backed data files. Use this for CSV, JSON, ROOT-linked, and text files in the personal file area when you need to inspect data, save analysis output, or stage a data file for later math work. Large files are previewed and sampled instead of being sent in full, so for CSV inspect the headers, top rows, and column profiles first, then feed those values into the transform or plot the user asked for. If a user says a .txt file is really CSV-format data, treat it as CSV if the content looks tabular. For physics datasets from the LHC, CMS, or ATLAS, assume the file contains real observed experimental data unless the user explicitly says simulated, and only mention sample counts or provenance that appear in the file preview or tool output. For ROOT files, look for a companion .manifest.json or .root.json file and use that hierarchy if it exists. For non-write actions, send content and contentType as empty strings. Reading returns file content or a compact preview plus a structural summary; writing stores a text file at the requested path.",
            parameters = new
            {
                type = "object",
                properties = new
