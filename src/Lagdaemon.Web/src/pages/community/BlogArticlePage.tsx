...
import type { BlogArticle, BlogComment } from '../../api/blogApi'
...
export default function BlogArticlePage({ slug, onNavigateBack, onNavigateEditor }: Props) {
  ...
  return (
    <div className="community-page blog-article-page">
      ...
      {article.coverUrl && <img src={article.coverUrl} alt={article.title} className="blog-article-cover" />}
        ...
        {(isAuthor || isAdmin) && (
          <div className="blog-article-actions">
            ...
            {isAdmin && article.status === 'submitted' && (
              <>
                <button className="primary-action" onClick={() => blogApi.setStatus(article.id, 'published').then(setArticle)}>Publish</button>
                <button onClick={() => blogApi.setStatus(article.id, 'rejected').then(setArticle)}>Reject</button>
              </>
            )}
          </div>
        )}
        ...
        {user ? (
          <form className="blog-comment-form" onSubmit={handleComment}>
            <textarea value={commentText} onChange={e => setCommentText(e.target.value)}
              placeholder="Add a comment…" rows={4} required style={{ width: '100%', resize: 'vertical' }} />
            <button type="submit" className="primary-action" disabled={submitting || !commentText.trim()}>
