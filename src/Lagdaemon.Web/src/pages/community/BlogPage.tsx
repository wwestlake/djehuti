...
import type { BlogSection, BlogArticle } from '../../api/blogApi'
  ...
  return (
    <div className="community-page">
      <div className="blog-header">
        ...
        {user && (
          <button className="primary-action" onClick={() => onNavigateEditor()}>Write Article</button>
        )}
      </div>
      ...
      <div className="blog-section-tabs">
        ...
        {sections.map(s => (
          <button key={s.id} className={`blog-tab${activeSection === s.id ? ' active' : ''}`} onClick={() => setActiveSection(s.id)}>
            {s.name}
          </button>
        ))}
      </div>
