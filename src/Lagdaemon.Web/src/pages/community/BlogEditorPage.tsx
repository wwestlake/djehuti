...
import type { BlogArticle, BlogSection } from '../../api/blogApi'
...
export default function BlogEditorPage({ articleId, onSaved, onCancel }: Props) {
  ...
  return (
    <div className="community-page blog-editor">
      <div className="blog-editor-toolbar">
        <button className="breadcrumb-link" onClick={onCancel}>← Cancel</button>
        <div className="blog-editor-toolbar-right">
          <button type="button" className={`blog-tab${preview ? ' active' : ''}`} onClick={() => setPreview(v => !v)}>
      ...
      <div className="blog-editor-form">
        ...
        <label className="blog-editor-label">
          ...
          <input type="text" value={title} onChange={e => setTitle(e.target.value)}
            placeholder="Article title" maxLength={200} />
        </label>
        <label className="blog-editor-label">
          Excerpt (optional)
          <input type="text" value={excerpt} onChange={e => setExcerpt(e.target.value)}
            placeholder="Short summary shown in listings" maxLength={300} />
        </label>
        <label className="blog-editor-label">Content (Markdown)</label>
        {preview ? (
          ...
          <textarea className="blog-editor-textarea" value={content}
            onChange={e => setContent(e.target.value)}
            placeholder="Write your article in Markdown…" rows={24} />
        )}
      </div>
    </div>
  )
