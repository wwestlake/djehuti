...
export const blogApi = {
  ...
  getArticles: (sectionId?: string, page = 1, pageSize = 20): Promise<BlogArticle[]> => {
    ...
    return fetch(`${BASE}/articles?${params}`).then(r => r.json())
  },

  getArticle: (slug: string): Promise<BlogArticle> =>
    fetch(`${BASE}/articles/${slug}`).then(r => r.json()),

  getMyArticles: (page = 1, pageSize = 20): Promise<BlogArticle[]> =>
    fetch(`${BASE}/my-articles?page=${page}&pageSize=${pageSize}`).then(r => r.json()),
  ...
  deleteArticle: (id: string): Promise<void> =>
    fetch(`${BASE}/articles/${id}`, { method: 'DELETE' }).then(() => {}),

  getComments: (articleId: string): Promise<BlogComment[]> =>
    fetch(`${BASE}/articles/${articleId}/comments`).then(r => r.json()),
  ...
  deleteComment: (id: string): Promise<void> =>
    fetch(`${BASE}/comments/${id}`, { method: 'DELETE' }).then(() => {}),
}
