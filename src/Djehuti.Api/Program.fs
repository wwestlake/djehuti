...
let main args =
    ...
    app.MapPost(
        "/api/admin/semantic/reindex/mud/items",
        Func<HttpContext, IResult>(fun ctx ->
            match tryGetAuthClaims ctx with
            | Some claims when Permissions.isAdmin claims.Role ->
                let indexed = SemanticGraphRepository.reindexMudItems()
                Results.Ok({| indexed = indexed |})
            | Some _ -> Results.Forbid()
            | None -> Results.Unauthorized()
        )
    ...
    app.MapPost(
        "/api/admin/semantic/reindex/mud/recipes",
        Func<HttpContext, IResult>(fun ctx ->
            match tryGetAuthClaims ctx with
            | Some claims when Permissions.isAdmin claims.Role ->
                let indexed = SemanticGraphRepository.reindexMudRecipes()
                Results.Ok({| indexed = indexed |})
            | Some _ -> Results.Forbid()
            | None -> Results.Unauthorized()
        )
    ...
    // ============================================================
    // RAG API Endpoints - Semantic Context Retrieval for all apps
    // ============================================================

    // 1. GET /api/semantic/app-context/{appName}
    // Query current version/checksum of app instructions
    app.MapGet(
        "/api/semantic/app-context/{appName}",
        Func<HttpContext, string, IResult>(fun ctx appName ->
            match tryGetAuthClaims ctx with
            | Some _ ->
    ...
    // Upload/update app instructions and examples
    app.MapPost(
        "/api/semantic/app-context/{appName}",
        Func<HttpContext, string, {| version: string; instructions: string; examples: string list; checksum: string |}, IResult>(fun ctx appName body ->
            match tryGetAuthClaims ctx with
            | Some claims ->
                match Guid.TryParse(claims.UserId) with
                | true, userId ->
    ...
    // Get combined app + user context for prompt building
    app.MapGet(
    ...
    app.MapPost(
        "/api/semantic/save-conversation",
        Func<HttpContext, {| appName: string; title: string; turns: {| role: string; content: string |} list |}, IResult>(fun ctx body ->
            match tryGetAuthClaims ctx with
            | Some claims ->
                match Guid.TryParse(claims.UserId) with
                | true, userId ->
    ...
    app.MapGet(
        "/api/semantic/conversations/{id}",
        Func<HttpContext, Guid, IResult>(fun ctx conversationId ->
            match tryGetAuthClaims ctx with
            | Some claims ->
                match Guid.TryParse(claims.UserId) with
                | true, userId ->
